...
to go to `System Settings -> Privacy & Security -> Accessibility` and add the script to the list of apps that are
allowed to control your computer.

This does not apply to the `ghostty` boot option: it launches tmux through Ghostty's own command-line interface
(`open -na Ghostty --args -e tmux`) rather than System Events, so it needs no Accessibility permission.
...
##### this will only start the tmux server, it will _not_ start any terminal emulator
